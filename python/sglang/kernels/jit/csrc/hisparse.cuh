...
__device__ __forceinline__ bool try_get_extra_page_device_loc(
  ...
  if constexpr (IsDsv4Layout) {
    ...
    for (
        int64_t candidate_slot = HOT_BUFFER_SIZE;
...
__global__ void load_cache_to_device_buffer_kernel(
  ...
  if (bid >= num_real_reqs[0]) {
    for (int i = tid; i < NUM_TOP_K; i += BLOCK_SIZE) {
      top_k_device_locs[bid * top_k_device_locs_stride + i] = -1;
    }
    return;
  }
  ...
  for (int64_t step = 0; step < num_steps; ++step) {
    const int32_t* req_top_k_tokens = req_top_k_tokens_base + step * NUM_TOP_K;
      ...
      for (int i = tid; i < NUM_TOP_K; i += BLOCK_SIZE) {
        ...
        if (i < count) {
          ...
          try_get_static_device_loc<HOT_BUFFER_SIZE, IsDsv4Layout>(
              token_pos, req_device_buffer_locs, page_size, &device_loc) ||
    ...
    // Insert top-k tokens into shared-memory hash table. Current speculative
    // tokens live in the extra page and are direct hits, not host misses.
    for (int i = tid; i < NUM_TOP_K; i += BLOCK_SIZE) {
