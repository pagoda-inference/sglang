...
constexpr uint32_t SPEC_UNIQUE_INDEX_MASK = 0xFFFFu;
...
__device__ bool try_get_active_tail_device_loc(
    int32_t token_idx,
    int64_t seq_len,
...
template <int HOT_BUFFER_SIZE>
__device__ int32_t hot_cache_lookup(
  ...
  for (int32_t attempt = 0; attempt < table_size; ++attempt) {
    ...
    if (static_cast<int32_t>(static_cast<uint32_t>(packed)) == token) {
      ...
      return static_cast<uint32_t>(index_entry >> 32) == scratch_epoch
                 ? static_cast<int32_t>(static_cast<uint32_t>(index_entry) & SPEC_UNIQUE_INDEX_MASK)
                 : -1;
    }
...
template <
    ...
    int HOT_BUFFER_SIZE,
    int NUM_STEPS>
__global__ void claim_hisparse_spec_misses_kernel(
  ...
  const int64_t global_warp = static_cast<int64_t>(blockIdx.y) * NUM_WARPS + tid / WARP_SIZE;
  const int64_t occ = static_cast<int64_t>(lane_id) * (gridDim.y * NUM_WARPS) + global_warp;
  const int64_t rid = req_pool_indices[bid];
  const int32_t* req_top_k_tokens = top_k_tokens + bid * top_k_tokens_stride;
  const int32_t* req_device_buffer_tokens = device_buffer_tokens + rid * buffer_stride_0;
  const int32_t* req_device_buffer_locs = device_buffer_locs + rid * buffer_stride_0;
  ...
  const bool hash_degraded =
      (static_cast<uint32_t>(*req_scratch_generation) & HASH_DEGRADED_FLAG) != 0;
  const uint32_t next_epoch =
      (static_cast<uint32_t>(*req_scratch_generation) & SCRATCH_EPOCH_MASK) + 1;
  const uint32_t scratch_epoch = next_epoch > SCRATCH_EPOCH_MASK ? 1 : next_epoch;
  ...
  if (occ < total_occurrences) {
    ...
    if (token >= 0 && token < seq_len) {
      ...
      if (try_get_active_tail_device_loc<HOT_BUFFER_SIZE>(
              token,
              seq_len,
              const_cast<int32_t*>(req_device_buffer_tokens),
              const_cast<int32_t*>(req_device_buffer_locs),
              active_tail_slots,
              &direct_loc)) {
  ...
  for (int32_t candidate_lane = 0; candidate_lane < WARP_SIZE; ++candidate_lane) {
    const int32_t candidate_token = __shfl_sync(FULL_WARP_MASK, token, candidate_lane);
    const int32_t candidate_needs_lookup =
        __shfl_sync(FULL_WARP_MASK, needs_cache_lookup ? 1 : 0, candidate_lane);
    if (candidate_needs_lookup != 0 && candidate_token == token && candidate_lane < lookup_owner_lane) {
      lookup_owner_lane = candidate_lane;
    }
  }
  if (!needs_cache_lookup || lane_id != lookup_owner_lane) return;

  const int32_t cache_slot = hot_cache_lookup<HOT_BUFFER_SIZE>(
      req_ring_hash_keys,
      req_ring_hash_vals,
      ...
      hash_degraded);
  if (cache_slot >= 0) return;
  const int64_t src_loc = req_host_cache_locs[token];
  if (src_loc < 0) return;

  int32_t hash_pos = hash_slot(token, static_cast<int>(total_occurrences));
  const auto epoch_bits = static_cast<unsigned long long>(scratch_epoch) << 32;
  const auto packed = epoch_bits | static_cast<uint32_t>(token);
  for (int64_t attempt = 0; attempt < total_occurrences; ++attempt) {
    auto old = atomicCAS(req_scratch_table + hash_pos, 0ull, 0ull);
    if (static_cast<uint32_t>(old >> 32) != scratch_epoch) {
      if (atomicCAS(req_scratch_table + hash_pos, old, packed) == old) {
        const int32_t unique_idx = atomicAdd(req_work_count, 1);
        req_compact_hash_positions[unique_idx] = hash_pos;
        __threadfence();
        atomicExch(
            req_scratch_indices + hash_pos,
            epoch_bits | (static_cast<uint64_t>(occ) << 16) | static_cast<uint32_t>(unique_idx));
        return;
      }
    } else if (static_cast<uint32_t>(old) == static_cast<uint32_t>(token)) {
      return;
    }
...
__global__ void load_cache_to_device_buffer_spec_gather_kernel(
  ...
  // One warp can contain multiple speculative steps for the same token. Elect a
  // single leader for every duplicate group so a producer can never spin while
  // its duplicate consumer is divergent in the same warp.
  int32_t lookup_owner_lane = lane_id;
  ...
  reuse_owner_loc = needs_cache_lookup && lane_id != lookup_owner_lane;
      ...
      if (src_loc >= 0) {
        ...
        for (int64_t attempt = 0; attempt < total_occurrences; ++attempt) {
          ...
          if (static_cast<uint32_t>(old >> 32) == static_cast<uint32_t>(scratch_epoch) &&
              static_cast<uint32_t>(old) == static_cast<uint32_t>(token)) {
            ...
            if (static_cast<uint32_t>(index_entry >> 32) == static_cast<uint32_t>(scratch_epoch)) {
              const uint32_t packed_index = static_cast<uint32_t>(index_entry);
              const int64_t owner_occ = static_cast<int64_t>(packed_index >> 16);
              const int32_t unique_idx =
                  static_cast<int32_t>(packed_index & SPEC_UNIQUE_INDEX_MASK);
              if constexpr (RecordMissPlan) {
                miss_idx = unique_idx;
              }
              copy_owner = occ == owner_occ;
  ...
  for (int64_t iteration = 0; iteration < compact_iterations; ++iteration) {
    ...
    if (compact_idx < miss_count) {
      ...
      if (static_cast<uint32_t>(packed >> 32) == static_cast<uint32_t>(scratch_epoch) &&
        ...
        unique_idx = static_cast<int32_t>(
            static_cast<uint32_t>(index_entry) & SPEC_UNIQUE_INDEX_MASK);
...
void load_cache_to_device_buffer_spec(
  ...
  LaunchKernel(dim3(static_cast<uint32_t>(bs), tiles), BLOCK_SIZE, device)(
      claim_hisparse_spec_misses_kernel<
          BLOCK_SIZE,
          NUM_TOP_K,
          HOT_BUFFER_SIZE,
          NUM_STEPS>,
